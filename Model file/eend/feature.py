...
def transform(
    ...
    if not transform_type:
        ...
        n_fft = 2 * (Y.shape[1] - 1)
        sr = 16000  ##input signal sampling rate
        n_mels = 40
        ...
        Y = Y / std
    elif transform_type == 'logmel23_new':
        n_fft = 2 * (Y.shape[1] - 1)
        sr = 8000
        n_mels = 23  
        mel_basis = librosa.filters.mel(sr, n_fft, n_mels)  
        mel = np.dot(Y, mel_basis.T)
        Y = 20 * np.log10(np.maximum(1e-5, mel))
        # normalize
        Y = np.clip((Y - 20 + 100) / 100, 1e-8, 1)
...
def stft(
    ...
    ##pre emphasis for pre processing
    if (pre_emphasis == True):
        pre_emphasis_ratio = 0.97
        emphasized_signal = np.append(data[0], data[1:] - pre_emphasis_ratio * data[:-1])
    else :
        emphasized_signal = data
...
def get_labeledSTFT(
    ...
    if (aug==True):
        ...
        augment = audiomentations.Compose([
                  ...
                  audiomentations.FrequencyMask(min_frequency_band=0.0, max_frequency_band=0.5, p=0.5)
                    ])
        #augmentation
        data = augment(data, rate)
    
    data = np.asarray(data)
    Y = stft(pre_emphasis,data, frame_size, frame_shift)
