...
def calc_diarization_error(pred, label, threshold, label_delay=0):
    """
    ...
    decisions = torch.sigmoid(pred[label_delay:, ...]) > threshold  ##modify here if u want to ignore overlap
    ...
    res['speech_falarm'] = ((n_ref == 0) & (n_sys > 0)).sum() ##if ignore overlap, this error become false alarm
    res['speaker_scored'] = (n_ref).sum()
    res['speaker_miss'] = torch.max((n_ref - n_sys), torch.zeros_like(n_ref)).sum()  ##if ignore overlap, this error become (miss + overlap)
    res['speaker_falarm'] = torch.max((n_sys - n_ref), torch.zeros_like(n_ref)).sum()  ##if ignore overlap, this error become false alarm, as same as "speech_falarm"
    n_map = ((label == 1) & (decisions == 1)).sum(axis=-1)
    res['speaker_error'] = (torch.min(n_ref, n_sys) - n_map).sum()
    res['correct'] = torch.true_divide((label == decisions).sum(),label.shape[1])  ##Eric modify 2021.1.21  / -> true_divide
...
def report_diarization_error(ys, labels, threshold):
    """
    ...
    for y, t in zip(ys, labels):
        stats = calc_diarization_error(y, t, threshold)
