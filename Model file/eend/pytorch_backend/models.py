...
class TransformerCNNModel(nn.Module): ##produce by Eric 2021/2/24
    def __init__(self, n_speakers, in_size, n_heads, n_units, n_layers, dim_feedforward=2048, dropout=0.5, has_pos=False):
        """ Self-attention-based diarization model.
        ...
        ##self.encoder = nn.Linear(in_size, n_units) ##modify by eric  from convolution --> 1d CNN
        self.encoder = nn.Conv1d(in_channels=in_size, out_channels=n_units,kernel_size=(context_size*2)+1, bias=True, padding=context_size)
        self.encoder_norm = nn.LayerNorm(n_units)
        if self.has_pos:
            self.pos_encoder = PositionalEncoding(n_units, dropout)
        encoder_layers = TransformerEncoderLayer(n_units, n_heads, dim_feedforward, dropout)
